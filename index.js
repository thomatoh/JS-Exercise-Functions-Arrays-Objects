...
const thomas = makePersonObject(1, 'thomas', 'mail@mail.com')
...
function getCarInfoByIndex(inventory, index) {
  const { car_make, car_model } = inventory[index];
...
function getModelYears(inventory) {
  let carYearsArray = [];
  for (var i = 0; i < inventory.length; i++) {
    carYearsArray.push(inventory[i].car_year)
  }
...
function getOlderCars(inventory, year) {
  ...
  for (var i = 0; i < inventory.length; i++) {
    if (inventory[i].car_year <= year) {
      olderCarsArray.push(inventory[i])
    }
  }
...
function getGermanCars(inventory) {
  ...
  const foundGermanCars = [];
  for (var i = 0; i < inventory.length; i++) {
    for (var j = 0; j < germanCars.length; j++) {
      if (inventory[i].car_make == germanCars[j]) {
        foundGermanCars.push(inventory[i])
      }
    }
  }
  return foundGermanCars;
}
